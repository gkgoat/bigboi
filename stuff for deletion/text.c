...
//Monochrome font data, each hexadecimal represents 4 rows / bytes of a 8x8 pixel character
//ok bruh. how we supposed to use dis
...
//Printing a string onto the screen by looking up each of its characters and plotting the binary data
...
//Draw text in mode 3
void m3_puts(int x, int y, const char *str, COLOR clr, int shadow, COLOR shadow_clr)
{
	if(shadow == 1)
	{
		bm16_puts(&vid_mem[(y + 1)*240+x + 1], str, shadow_clr, 240);
	}
